...
# Prompt for Firebase Studio: Create a Native Android App for UtsavLook

## I. Core App Concept & Objective

Create a native Android application for "UtsavLook," a platform for discovering and booking mehndi and makeup artists. The app must replicate the full functionality of the existing UtsavLook web application, providing three distinct user experiences (Customer, Artist, and Admin) and utilizing Firebase for its backend services (Authentication, Firestore Database).
...
The entire app must adhere to the UtsavLook brand identity.
...
*   **TopAppBar:** Display the "UtsavLook" brand name and a user profile icon that opens an account management screen.
